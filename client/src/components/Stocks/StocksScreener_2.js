...
import { Tabs, Tab, TabContent, Button, Modal } from 'react-bootstrap';
...
export default class StocksScreener extends React.Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      console.log("stocks data response: " + stocksDataJson.data );
      const stocksList = [];
      Object.keys(stocksDataJson.data.stocks).map((symbol) => {
        var data = stocksDataJson.data.stocks[symbol];
        stocksList.push(constructStockJson(data));
      });
      this.setState({ tableProps: tableProps(stocksList) });


      var categoryStocksMapperData = []
      Object.keys(categoryStocksMapper.data).map(category => {
        categoryStocksMapperData.push({
          category: category,
          stocksListJson: categoryStocksMapper.data[category]})
      })
